...
class BaseSSL(nn.Module):
    ...
    IMAGENET_PATH = os.environ.get('IMAGENET_PATH', '/home//imagenet/raw-data/')
...
class SimCLR(BaseSSL):
    ...
    def __init__(self, hparams, device=None):
        ...
        elif self.hparams.method == 'new_positive':
            self.criterion = models.losses.NTXent_newp(
                tau=hparams.temperature,
                multiplier=hparams.multiplier,
                distributed=(hparams.dist == 'ddp'),
            )
        elif self.hparams.method == 'drop_easy_negative':
            self.criterion = models.losses.NTXent_den(
                tau=hparams.temperature,
                multiplier=hparams.multiplier,
                distributed=(hparams.dist == 'ddp'),
            )
        elif self.hparams.method == 'drop_false_negative':
            self.criterion = models.losses.NTXent_dfn(
                tau=hparams.temperature,
                multiplier=hparams.multiplier,
                distributed=(hparams.dist == 'ddp'),
            )
        elif self.hparams.method == 'drop_false_positive':
            self.criterion = models.losses.NTXent_dfp(
                tau=hparams.temperature,
                multiplier=hparams.multiplier,
                distributed=(hparams.dist == 'ddp'),
            )
        ...
        self.clip_model,preprocess = clip.load("ViT-B-32.pt",device=device)
    ...
    def step(self, batch):
        ...
        loss, acc = self.criterion(z,z_clip)
        return {
            'loss': loss,
            'contrast_acc': acc,
        }
...
class SSLEval(BaseSSL):
    ...
    def train_step(self, batch, it=None,d_ratio = 0, u_ratio=0):
        logs = self.step(batch)
        if it is not None:
